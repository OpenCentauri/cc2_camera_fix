...
STATUS=FAIL
OVERWRITE=@OVERWRITE@
OLD_DIR=/tmp/.cc2-old-scripts-$TOKEN
...
known_or_absent() {
    ...
    if [ -e "$dest" ]; then
        ...
        case "$comparison" in
            0) ;;
            1)
                [ "$OVERWRITE" = 1 ] && [ "$MODE" = install ] || managed_failure content
                case "$dest" in "$CONFIG/system.sh"|"$CONFIG/enabled/10-erase-fix.sh") ;; *) managed_failure content;; esac
                ;;
...
install_files() {
    ...
    if [ -e "$CONFIG/enabled/10-erase-fix.sh" ] && [ -e "$CONFIG/system.sh" ] && busybox cmp -s "$STAGE/fix" "$CONFIG/enabled/10-erase-fix.sh" && busybox cmp -s "$STAGE/runner" "$CONFIG/system.sh"; then
        STATUS=SAME
        return
    fi
    ...
    [ "$(busybox md5sum /dev/mtd5)" = "$baseline" ] || fail config-changing
    if [ "$OVERWRITE" = 1 ]; then
        mkdir "$OLD_DIR" || fail incomplete-install
        for n in fix runner; do
            case "$n" in fix) dest=$CONFIG/enabled/10-erase-fix.sh;; runner) dest=$CONFIG/system.sh;; esac
            if [ -e "$dest" ]; then
                cp -p "$dest" "$OLD_DIR/$n" || fail copy
                busybox cmp -s "$dest" "$OLD_DIR/$n" || fail config-changing
            fi
        done
        [ "$(busybox md5sum /dev/mtd5)" = "$baseline" ] || fail config-changing
    fi
    ...
    # Feature first, boot entry point last. Replacement requires explicit consent.
    for n in fix runner; do
        case "$n" in
        ...
        sync || fail sync
        if [ -e "$dest" ]; then
            [ "$OVERWRITE" = 1 ] && regular "$dest" && [ "$(busybox stat -c %a "$dest")" = 755 ] || fail destination-appeared
            busybox cmp -s "$OLD_DIR/$n" "$dest" || fail config-changing
        else
